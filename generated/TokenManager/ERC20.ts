...
export class ERC20 extends ethereum.SmartContract {
  ...
  ethTokenAddr(): Address {
    let result = super.call("ethTokenAddr", "ethTokenAddr():(address)", []);

    return result[0].toAddress();
  }

  try_ethTokenAddr(): ethereum.CallResult<Address> {
    let result = super.tryCall("ethTokenAddr", "ethTokenAddr():(address)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
